...
const useGetList = () => {
  ...
  useEffect(() => {
    ...
    const getList = async () => {
      // axios请求数据
      const res = await axios.get('http://localhost:5505/list')
...
const App = () => {
  ...
  // tab切换功能
  // 1、点击谁就把谁的type记录下来
  // 2、通过记录的type和每一项遍历时type做匹配，控制激活类名的显示
      ...
      <div className="reply-navigation">
        <ul className="nav-bar">
          ...
          <li className="nav-sort">
            ...
            {tabs.map(item =>
              <span key={item.type}
                onClick={() => handleTabChange(item.type)} className={classNames('nav-item', {active: type === item.type})}>{item.text}
              </span>)}
          </li>
        </ul>
      </div>
